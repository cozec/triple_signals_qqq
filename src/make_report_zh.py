...
from make_report import g, avg, b64, pct, table, COLOR, SIG, ABL, TRIG, ARTICLE, m, CSS, ROOT
...
<h2>近期信号事件 —— 两个案例</h2>
<p>这些信号在最近的真实行情里长什么样？两张放大图把机制讲清楚。每张图的四个面板分别是：
QQQ 价格、CAPE 分位、回撤（及 25 日收益）、VIX，并画出各自的决策阈值线。</p>

<h3>A）2022–23 熊市 → 最近一次买入触发</h3>
<img src="data:image/png;base64,{b64('event_2023_bottom.png')}" alt="2022-23 熊市与 2023-05 买入触发">
<p>2022 年的熊市把 QQQ 的回撤打到 <b>−20%“深跌”</b>线以下，并在 2022 到 2023 的大部分时间里
一直待在那儿——这一个低位信号让框架持续处于<b>小底</b>模式（加仓至约 75% TQQQ）。但它从未升级为
<b>大底</b>：CAPE 分位最低只到约 0.45（从未 &lt;20% “便宜”），VIX 最高约 33（从未 &gt;40 “恐慌”），
所以三个信号里始终只亮一个。绿线标的是 <b>2023-05</b>，即回撤回升前、平滑回撤最后一次仍低于 −20% 的月份——
也是整个 2000–2026 样本里最近一次由信号驱动的买入。</p>

<h3>B）2026 年 3–4 月的回调</h3>
<img src="data:image/png;base64,{b64('event_2026_correction.png')}" alt="2026 年 3-4 月回调">
<p>一次<b>约 −16% 的快速回调</b>（原始峰谷值；策略读取的 5 日平滑回撤约 −9%）在 <b>2026-03-30</b> 见底，
VIX 仅升至约 28–31——偏高但够不着 40 的恐慌线，CAPE 也远谈不上便宜。于是<b>没有任何低位信号</b>触发：
没有抄底，没有梭哈 TQQQ。唯一的影响是这次下跌把指数打离高点，从而<i>关闭</i>了 3–4 月的
“高位贴近新高 → 不追”减仓状态，让框架在回调中<b>恢复正常买入</b>。随后是快速的 V 形反弹
（25 日收益到 5 月已摆到 +19%），把 QQQ 推上新高；CAPE 重新钉在上限，策略又回到“不追”状态。
这是一次教科书式的“浅而快”的回调——正是框架不必抄底、顺势穿过的那种。</p>

<h2>信号到底触发了多少次？</h2>
<p>在 <b>{TRIG['N']} 个月度决策日</b>（2000 年 1 月 → 2026 年 6 月）上，用两种方式统计：
信号亮着的总<b>月数</b>，以及形成的不同<b>episode（连续区段）数</b>——后者更接近“分别触发了多少次”。</p>
<div class="card">{table(["信号 / 动作","亮着月数","触发次数(episode)"],[
  [f'<span class="dot" style="background:#1f77b4"></span>CAPE 便宜（&lt;20%）', TRIG['rows']['cheap'][0], f"<b>{TRIG['rows']['cheap'][1]}</b>"],
  [f'CAPE 高估（&gt;70%）', TRIG['rows']['high'][0], TRIG['rows']['high'][1]],
  [f'CAPE 泡沫（&gt;85%）', TRIG['rows']['bubble'][0], TRIG['rows']['bubble'][1]],
  [f'<span class="dot" style="background:#2ca02c"></span>DD 深跌（&lt;−20%）', TRIG['rows']['deep'][0], f"<b>{TRIG['rows']['deep'][1]}</b>"],
  [f'25 日急跌（&lt;−12%）', TRIG['rows']['crash'][0], f"<b>{TRIG['rows']['crash'][1]}</b>"],
  [f'<span class="dot" style="background:#d62728"></span>VIX 恐慌（&gt;40）', TRIG['rows']['panic'][0], f"<b>{TRIG['rows']['panic'][1]}</b>"],
  [f'VIX 平静（&lt;12）', TRIG['rows']['calm'][0], TRIG['rows']['calm'][1]],
  [f'<b>买入 —— 大底（≥2 个低位 → 梭哈 TQQQ）</b>', f"<b>{TRIG['rows']['major'][0]}</b>", f"<b>{TRIG['rows']['major'][1]}</b>"],
  [f'买入 —— 小底（1 个低位 → 加仓）', TRIG['rows']['minor'][0], TRIG['rows']['minor'][1]],
  [f'减仓 —— “不追”修剪', TRIG['rows']['nochase'][0], TRIG['rows']['nochase'][1]],
])}</div>
<div class="callout warn"><b>要点。</b>
<ul style="margin:8px 0 0">
 <li>最关键的动作——<b>大底（梭哈 TQQQ）</b>——在 26 年里只触发了 <b>{TRIG['rows']['major'][1]} 次</b>
   （共 {TRIG['rows']['major'][0]} 个月，主要集中在 2008–2011 那一长段）。最后一次是 <b>{TRIG['last_major']}</b> 年，此后再无。</li>
 <li><b>月数 ≠ 次数。</b>DD 深跌共 {TRIG['rows']['deep'][0]} 个月，却只有 <b>{TRIG['rows']['deep'][1]} 个 episode</b>——
   QQQ 一旦水下就会连续好几年（是一种持续的<i>状态</i>，而非频繁触发）。25 日急跌正相反：
   {TRIG['rows']['crash'][0]} 个月却分成 <b>{TRIG['rows']['crash'][1]} 次</b>——这才是真正的“新崩盘”探测器。</li>
 <li><b>VIX 恐慌</b>是最稀有的真实触发：{TRIG['rows']['panic'][0]} 个月 / <b>{TRIG['rows']['panic'][1]} 次</b>（2008、2011、2020）。</li>
 <li>按触发次数算，<b>防御性的“不追”修剪是最频繁的动作（{TRIG['rows']['nochase'][1]} 次）</b>——
   而且在近几年几乎是唯一在触发的，因为 CAPE 一直钉在上限。</li>
</ul>
信号在<i>防御</i>方向上不停触发，但整套策略赖以成立的激进抄底却极其稀有——<b>26 年里仅 {TRIG['rows']['major'][1]} 次</b>。
这正是为什么实际收益主要来自那条一直在场的 TQQQ 底仓，而非择时。</div>
